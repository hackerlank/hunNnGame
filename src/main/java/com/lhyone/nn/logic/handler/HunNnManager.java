...
public class HunNnManager {
	...
	public static void joinRoom(HunNnBean.ReqMsg reqMsg, ChannelHandlerContext ctx) throws Exception {
		...
		try {
			...
			rspData.addAllPosition(NnUtil.getPositionInfo(reqMsg.getRoomNo()));
	...
	private static int getRestTime(String roomNo) {
		...
		if (curStatus.equals(NnRoomMatchStatusEnum.PLAY_GAME_STATUS.getCode() + "")) {
			restTime = (int) (NnConstans.PLAY_GAME_TIME - (System.currentTimeMillis() - timeOutVo.getPlayGameTime()) / 1000);
		}
		if (curStatus.equals(NnRoomMatchStatusEnum.SHOW_MATCH_RESULT_STATUS.getCode() + "")) {
			restTime = (int) (NnConstans.SHOW_MATCH_RESULT_TIME - (System.currentTimeMillis() - timeOutVo.getShowMatchResultTime()) / 1000);
		}
		if (curStatus.equals(NnRoomMatchStatusEnum.GAME_IDLE_STATUS.getCode() + "")) {
			restTime = (int) (NnConstans.GAME_IDLE_TIME - (System.currentTimeMillis() - timeOutVo.getIdleTime()) / 1000);
		}
		return restTime;
	}
	...
	public static void applyLandlord(HunNnBean.ReqMsg reqMsg, ChannelHandlerContext ctx) {
		...
		boolean flag = RedisUtil.hexists(NnConstans.NN_ROOM_LANDLORD_APPLY_QUEUE_CACHE + reqMsg.getRoomNo(), reqMsg.getUserId() + "");
		...
		// 增加坐庄队列
		RedisUtil.sadd(NnConstans.NN_ROOM_LANDLORD_APPLY_QUEUE_CACHE + reqMsg.getRoomNo(), reqMsg.getUserId() + "");
		...
		pushsingle(rspMsg.build(), ctx);
		// 判断当前是否有庄家
		if (!NnUtil.checkIsHasLandlord(reqMsg.getRoomNo())) {
	...
	public static void setLandlord(String roomNo) {
		...
		if (RedisUtil.smembers(NnConstans.NN_ROOM_LANDLORD_APPLY_QUEUE_CACHE + roomNo).size() <= 0) {
			return;
		}
		...
		String landlordUserId = RedisUtil.smembers(NnConstans.NN_ROOM_LANDLORD_APPLY_QUEUE_CACHE + roomNo).iterator().next();

		ReqMsg.Builder reqMsg = ReqMsg.newBuilder().setUserId(Long.parseLong(landlordUserId)).setRoomNo(roomNo);

		// 移除庄家的申请队列
		RedisUtil.srem(NnConstans.NN_ROOM_LANDLORD_APPLY_QUEUE_CACHE + reqMsg.getRoomNo(), reqMsg.getUserId() + "");
		...
		// 增加准备倒计时
		{
			...
			// 增加个人准备倒计时
			{
				executorTask.schedule(new MyTimerTask(reqMsg.build(), NnTimeTaskEnum.USER_REDAY_TIME.getCode(), userRedayTime), NnConstans.USER_REDAY_TIME, TimeUnit.SECONDS);
			}
			userRedayTime = getRedayTime(reqMsg.getRoomNo(), String.valueOf(reqMsg.getUserId()));
			userInfo.setRedayTime((int) userRedayTime);
		}
	...
	public static void landlordReday(HunNnBean.ReqMsg reqMsg, ChannelHandlerContext ctx) {
		...
		timeout.setIdleTime(System.currentTimeMillis());
		...
		RedisUtil.hset(NnConstans.NN_ROOM_PRE, reqMsg.getRoomNo() + "", JsonFormat.printToString(roomInfo.build()));
		...
		// 更新当前状态为空闲状态
		RedisUtil.hset(NnConstans.NN_ROOM_CUR_STATUS_PRE, reqMsg.getRoomNo(), NnRoomMatchStatusEnum.GAME_IDLE_STATUS.getCode() + "");

		// 设置发牌倒计时
		{
			executorTask.schedule(new MyTimerTask(reqMsg, NnTimeTaskEnum.GAME_IDLE_TIME.getCode(), timeout.getPlayGameTime()), NnConstans.GAME_IDLE_TIME, TimeUnit.SECONDS);
	...
	public static void farmerAddChip(HunNnBean.ReqMsg reqMsg, ChannelHandlerContext ctx) {
		...
		// 判断用户金币是否充足
		HunNnBean.UserInfo.Builder userInfo = getCurUser(reqMsg.getUserId(), reqMsg.getRoomNo());
		long userGold = NnManagerDao.instance().getUserGold(userInfo.getUserId());

		if (userGold < reqMsg.getChipGold()) {
			rspMsg.setCode(NnRspCodeEnum.$1002.getCode());
			rspMsg.setMsg(NnRspCodeEnum.$1003.getMsg());
			pushsingle(rspMsg.build(), ctx);
			return;
		...
		if (!position.getUidsList().contains(userInfo.getUserId())) {
			position.getUidsList().add(userInfo.getUserId());
		}
		// 更新位置缓存
		...
		userInfo.setUserGold(userGold - reqMsg.getChipGold());
	...
	public static void sendCard(HunNnBean.ReqMsg reqMsg) {
		...
		try {
			...
			// 清除准备倒计时
			GameTimoutVo timeout = new GameTimoutVo();
			timeout = getGameTimoutVo(reqMsg.getRoomNo());
			timeout.setPlayGameTime(0);
			RedisUtil.hset(NnConstans.NN_REST_TIME_PRE, reqMsg.getRoomNo(), JSONObject.toJSONString(timeout));
			...
			String[] sers = new String[] { "", "2", "3", "4", "5" };
			...
			// 增加展示比赛结果定时任务
			{
				executorTask.schedule(new MyTimerTask(reqMsg, NnTimeTaskEnum.SHOW_MATCH_RESULT_TIME.getCode(), timeout.getShowMatchResultTime()), NnConstans.SHOW_MATCH_RESULT_TIME, TimeUnit.SECONDS);
			...
			// 计算比赛结果
			executor.execute(new NnWork(reqMsg, NnWrokEnum.SHOW_MATCH_RESULT.getCode()));
	...
	public static void showMatchResult(String roomNo) {
		...
		List<HunNnBean.UserInfo.Builder> listUser = new ArrayList<HunNnBean.UserInfo.Builder>();

		calculation(listUser, roomNo, roomInfo, landlordUser, landlordDouble, landlordPosition, position2);
		calculation(listUser, roomNo, roomInfo, landlordUser, landlordDouble, landlordPosition, position3);
		calculation(listUser, roomNo, roomInfo, landlordUser, landlordDouble, landlordPosition, position4);
		calculation(listUser, roomNo, roomInfo, landlordUser, landlordDouble, landlordPosition, position5);
		...
		List<HunNnBean.UserInfo.Builder> newListUser = new ArrayList<HunNnBean.UserInfo.Builder>();

		if (landlordUser.getTotalGold() + landlordUser.getWinGold() < 0) {
			// 计算闲家赢家用户的总金币数
			int userWinTotalGold = 0;
			for (HunNnBean.UserInfo.Builder user : listUser) {

				if (user.getWinGold() >= 0) {
					userWinTotalGold = userWinTotalGold + user.getWinGold();
				}
			}
			...
			for (HunNnBean.UserInfo.Builder user : listUser) {
				...
				if (user.getWinGold() >= 0) {
					...
					int m = (int) Math.floor((user.getWinGold() / userWinTotalGold) * landlordUser.getTotalGold());
					userRealWinTotalGold = userRealWinTotalGold + m;
					user.setWinGold(m);
					user.setTotalGold(user.getTotalGold() + m);
					user.setUserGold(user.getUserGold() + m);
				} else {
					user.setTotalGold(user.getWinGold() + user.getTotalGold());
					user.setUserGold(user.getUserGold() + user.getWinGold());
				}
			...
			landlordUser.setUserGold(landlordUser.getUserGold() + landlordUser.getWinGold());
			newListUser.add(landlordUser);
		}

		// 更新缓存
		for (HunNnBean.UserInfo.Builder user : newListUser) {
			RedisUtil.hset(NnConstans.NN_ROOM_USER_INFO_PRE + roomNo, user.getUserId() + "", JsonFormat.printToString(user.build()));
		...
		for (HunNnBean.UserInfo.Builder user : newListUser) {
			...
			dbVo.setWinGold(user.getWinGold());
	 ...
	 */
	private static void calculation(List<HunNnBean.UserInfo.Builder> listUser, String roomNo, HunNnBean.RoomInfo.Builder roomInfo, HunNnBean.UserInfo.Builder landlordUser, int landlordDouble, HunNnBean.PositionInfo.Builder landlordPosition,
		...
		for (long userId : farmerPosition.getUidsList()) {
			HunNnBean.UserInfo.Builder uInfo = getCurUser(userId, roomNo);
			// 积分计算[(庄家底分+闲家底分)*牛的倍数]
			// 底分
			int baseScore = landlordUser.getBaseGold() + uInfo.getBaseGold();
			// 庄家赢
			if (matchResult) {
	...
	private static int getRedayTime(String roomNo, String userId) {
		...
		if (RedisUtil.hexists(NnConstans.NN_ROOM_USER_REDAY_TIME_PRE + roomNo, userId)) {
			...
			redayTime = (int) (NnConstans.USER_REDAY_TIME - (System.currentTimeMillis() - Long.parseLong(redayTimeStr)) / 1000);
			if (redayTime <= 0) {
				redayTime = 0;
			}
		}
	...
	public static void initRoom(HunNnBean.ReqMsg reqMsg, ChannelHandlerContext ctx) {
		...
		rspData.setLandlord(landLordUser);
		rspData.setCurRoomStatus(Integer.parseInt(curStatus));
		rspMsg.setOperateType(NnPushMsgTypeEnum.INIT_ROOM_PUSH.getCode());
		rspMsg.setData(rspData);
		rspMsg.setCode(NnRspCodeEnum.$0000.getCode());
		rspMsg.setMsg(NnRspCodeEnum.$0000.getMsg());
	...
	public static void landlordExitRoom(HunNnBean.ReqMsg reqMsg, ChannelHandlerContext ctx) {
		...
		if (NnRoomMatchStatusEnum.INIT_HOME_STATUS.getCode() == Integer.parseInt(curStatus) || NnRoomMatchStatusEnum.GAME_IDLE_STATUS.getCode() == Integer.parseInt(curStatus)) {
	...
	public static void setLandlordTimerTask(HunNnBean.ReqMsg reqMsg) {
		...
		landlordPosition.clearCard().clearPerGold().clearTotalGold();
		position2.clearCard().clearPerGold().clearTotalGold().clearUids();
		position3.clearCard().clearPerGold().clearTotalGold().clearUids();
		position4.clearCard().clearPerGold().clearTotalGold().clearUids();
		position5.clearCard().clearPerGold().clearTotalGold().clearUids();
		...
		for (String key : allUser) {
			...
			if (key.equals(landlordUserId)) {
				curUser.clearBaseGold().clearCard().clearTotalGold().clearPerGold();

			} else {
				curUser.clearBaseGold().clearCard().clearTotalGold().clearPerGold().clearPlayerType();
			}
		...
		if (NnConstans.NN_LANDLORD_MIN_GOLD > landlordUser.getUserGold()||landlordUser.getLandlordTimes()>=NnConstans.NN_LANDLORD_TIMES) {
			...
			// 增加准备倒计时
			{
				...
				// 增加个人准备倒计时
				{
					executorTask.schedule(new MyTimerTask(reqMsg, NnTimeTaskEnum.USER_REDAY_TIME.getCode(), userRedayTime), NnConstans.USER_REDAY_TIME, TimeUnit.SECONDS);
				}
				userRedayTime = getRedayTime(reqMsg.getRoomNo(), String.valueOf(reqMsg.getUserId()));
				userInfo.setRedayTime((int) userRedayTime);
			}
	...
	public static void idleTimeTask(HunNnBean.ReqMsg reqMsg) {
		...
		// 设置空游戏状态
		GameTimoutVo timeout = new GameTimoutVo();
		timeout.setPlayGameTime(System.currentTimeMillis());
		...
		// 设置发牌倒计时
		{
			executorTask.schedule(new MyTimerTask(reqMsg, NnTimeTaskEnum.PLAY_GAME_TIME.getCode(), timeout.getPlayGameTime()), NnConstans.PLAY_GAME_TIME, TimeUnit.SECONDS);
	...
	public static void colseRoom(String roomNo){
		...
		// 更新当前状态为空闲状态
		String curStat=RedisUtil.hget(NnConstans.NN_ROOM_CUR_STATUS_PRE, reqMsg.getRoomNo());
