...
public class NnManagerDao {
	...
	public boolean addDb(List<DbVo> list) {
		...
		try {
			...
			for (DbVo bean : list) {
				...
				String sql4 = "insert into `nn_room_match_user` (`user_id`, `room_id`, `room_no`, `match_num`, `total_gold`, `base_gold`, `cost_gold`, `player_role`, `cards`, `card_type`, `is_win`, `doublex`,`order_no`, `create_date`)"
						...
						+ DateUtil.format(nrmu.getCreateDate(), DatePattern.NORM_DATETIME_PATTERN) + "')";
	...
	public long checkNnUserIsJoinRoom(long userId) {
		...
		long i = DbUtil.getInstance().getValue(sql, new Object[] { userId, GameTypeEnum.NIU_NIU.getType() });
		return i;
	}
}
