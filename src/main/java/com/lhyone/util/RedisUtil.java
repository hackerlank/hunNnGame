...
public class RedisUtil {
    ...
    private static JedisPool initializePool() {
        if (pool == null) {
            ...
            config.setMaxTotal(Integer.valueOf(prop.getProperty("redis.pool.maxTotal").trim()));
            //控制一个pool最多有多少个状态为idle(空闲的)的jedis实例。
            config.setMaxIdle(Integer.valueOf(prop.getProperty("redis.pool.maxIdle").trim()));
            //表示当borrow(引入)一个jedis实例时，最大的等待时间，如果超过等待时间，则直接抛出JedisConnectionException；
            config.setMaxWaitMillis(Integer.valueOf(prop.getProperty("redis.pool.maxWaitMillis").trim()));
            ...
            String redisIp = prop.getProperty("redis.ip").trim();
            ...
            String pw =null;
            if(!StringUtil.isNullOrEmpty(prop.getProperty("redis.password"))) {
                pw = prop.getProperty("redis.password").trim();
            }

            int dbIndex=Integer.parseInt(prop.getProperty("redis.databases"));
            pool = new JedisPool(config, redisIp, redisPort,2000,pw,dbIndex);
...
//    	RedisUtil.sadd("ids","1");
//    	RedisUtil.sadd("ids","2");
//    	RedisUtil.sadd("ids","3");
//    	RedisUtil.sadd("ids","4");
//    	RedisUtil.sadd("ids","5");
//    	String str=RedisUtil.smembers("idss").iterator().next();
//    	System.out.println(str);
    	
    	String roomStr = RedisUtil.hget(NnConstans.NN_ROOM_PRE + "200120", "roomInfo");
